...
def execute_deployment(
    *,
    ...
    # 2. (可選) 檢查 Host IDs 是否都存在
    # ...
    
    # 3. 在資料庫中建立一個 "pending" 狀態的 Job
...
def get_deployment_status_and_logs(
    *,
    ...
    """
    db_job = db.query(models.DeployJob).filter(models.Job.id == job_id).first()
    if not db_job:
